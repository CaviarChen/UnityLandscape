...
Terrain colouring:
    ...
    3. For each vertices, add the right colour based on height. (We also uses Random to add some noise)

Sea Generation:
    1. Generate a flat huge square at the sea level.

Collision:
    1. Adding a invisible sphere outside the camera.
    2. Using Unity built-in physics engine to prevent two object penetrate.
    3. Locking camera rotation and keep resetting the velocity of the camera to prevent the terrain bounce off the camera.

Shader:
    1. Modified based on the Phong Shader from workshop.
    (Since this is a terrain, we were trying to reduce the specular part. However, according to the suggestion from a tutor we asked, we leaved a quite obvious specular highlight)
